...
case_list = \
    [
        RandomWalker([[5, 5]], [80]),
        RandomWalker([[5, 5], [3, 7]], [80, 80]),
        RandomWalker([[5, 5], [3, 7], [0, 8]], [80, 80, 100])
    ]
...
if __name__ == '__main__':
    ...
    for randonWalker in case_list:
        random_walker_init_pos = randonWalker.random_walker_init_pos
        prob_for_the_deepest_value = randonWalker.prob_for_the_deepest_value
        ...
        if save_animation:
            ...
            fileExtender = saved_name.split(".")[1]
            fileName = "_".join((saved_name.split(".")[0], "case", str(case_num)))
            ani.save(".".join((fileName, fileExtender)), writer=writer)
            plt.close()
        else:
            plt.show()
